...
class BasicPattern(object):
    ...
    def panel_order(self, force_update=False):
        ...
        # remove None in panel_order
        self.pattern['panel_order'] = [panel for panel in self.pattern['panel_order'] if panel is not None]
    ...
    def _edge_as_vector(self, vertices, edge_dict):
        ...
        edge_type = [0, 0]  # default -- straight edge
        if ('curvature' in edge_dict):
            if isinstance(edge_dict['curvature'], list) or edge_dict['curvature']['type'] == 'quadratic':  
                curvature = np.concatenate([edge_dict['curvature']['params'][0], [0, 0, 0, 0], [0, 0, 0]])
                edge_type = [0, 1]
            elif edge_dict['curvature']['type'] == 'circle':
                if edge_dict['curvature']['params'][0] > 1000:
                    radius = 950 + 10 * np.random.rand()
                    edge_dict['curvature']['params'][0] = radius
                curvature = np.concatenate([[0, 0], [0, 0, 0, 0], edge_dict['curvature']['params']])
                edge_type = [1, 0]
            elif edge_dict['curvature']['type'] == 'cubic':
                curvature = np.concatenate([[0, 0], edge_dict['curvature']['params'][0] + edge_dict['curvature']['params'][1], [0, 0, 0]])
                edge_type = [1, 1]
            else:
                raise NotImplementedError(f'{self.__class__.__name__}::Unknown curvature type {edge_dict["curvature"]["type"]}')
        else:
            curvature = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        if ('label' in edge_dict):
            attach_label = edge_dict['label']
            if attach_label == 'lower_interface':
                ...
                edge_type += [1, 0, 0]
            elif attach_label == 'right_armhole':
                edge_type += [1, 0, 1]
            elif attach_label == 'left_armhole':
                edge_type += [1, 1, 0]
            else:
                raise NotImplementedError(f'{self.__class__.__name__}::Unknown edge label {attach_label}')
        else:
            edge_type += [0, 0, 0]

        return np.concatenate([edge_vector, curvature, edge_type])
