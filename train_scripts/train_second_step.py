...
from diffusion.model.nets import MultiModalNet, SewingLDM
...
def log_validation(model, step, device, save_folder=None, cfg_scale=2, tokenizer=None):
    ...
    null_y = null_y.unsqueeze(0).repeat(bs, 1, 1, 1)
    sketch = batch['sketch'].repeat(1, 1, 1, 1) # or null
    null_sketch = torch.zeros_like(sketch)
    sketch = torch.cat([sketch, null_sketch], dim=0)
    body_params = 0.5 * batch['body_params'].unsqueeze(1).repeat(2, 1, 1)
    c = {'sketch': sketch, 'body_params': body_params}

    z = torch.randn(bs, config.model_token_size, config.in_channels,  device=device).repeat(2, 1, 1)
    ...
    if random.random() < 0.5:
        ...
        free_text = False
        if load_text_feat:
            caption_embs = y_condition['caption_feature']
            emb_masks = y_condition['attention_mask'].to(torch.int16)
        else:
            ...
            emb_masks = txt_tokens.attention_mask[:, None, None]
    
    model_kwargs = dict(y=torch.cat([caption_embs, null_y]), data_info={},
    ...
    latent = samples.to(torch.float16)

    recovered_cloth = latent.reshape(bs, max_panel_num, max_edge_num, -1)
    discrete = recovered_cloth[..., -10:]
    # convert discrete: negative to zero, positive to 1
...
def train():
    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
                ...
                if not load_tokenizer_feat:
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast(enabled=(config.mixed_precision == 'fp16' or config.mixed_precision == 'bf16')):
                            x_input = tokenizer.encode_vector(input_data) * config.scaling_factor
                else:
            ...
            sketch = batch['sketch'].requires_grad_(True)
            body_params = 0.5 * batch['body_params'].unsqueeze(1).requires_grad_(True)
            c = {'sketch': sketch, 'body_params': body_params}
            ...
            with accelerator.accumulate(model):
                ...
                optimizer.zero_grad()
                loss_term, _ = train_diffusion.training_losses(model, x_input, timesteps, model_kwargs=dict(y=y, mask=mask, c=c))
            ...
            if (step + 1) % config.log_interval == 0 or (step + 1) == 1:
                ...
                info += f's:({model.module.base_model.token_size}), ' if hasattr(model, 'module') else f's:({model.base_model.token_size}), '
            ...
            if config.visualize and (global_step % config.eval_sampling_steps == 0):
                ...
                if accelerator.is_main_process:
                    ...
                    log_validation(model, global_step, device=accelerator.device, save_folder=os.path.join(config.work_dir, f'visualize_{global_step}'), tokenizer=tokenizer)
                    datawrapper.dataset.set_training(True)
        save_epoch = False
        ...
        accelerator.wait_for_everyone()
        if not save_epoch and accelerator.is_main_process:
            os.umask(0o000)
            save_checkpoint(os.path.join(config.work_dir, 'checkpoints'),
                            epoch=epoch,
                            step=global_step,
...
if __name__ == '__main__':
    ...
    if config.load_from is not None and args.resume_from is None:
        missing, unexpected = load_checkpoint(config.load_from, dit)
        logger.warning(f'Missing keys: {missing}')
        logger.warning(f'Unexpected keys: {unexpected}')
    
    dit.requires_grad_(True)
    pattern = re.compile(config.dit_trainable_param_pattern)
    ...
    if config.multimodalnet_path:
        logger.info("Loading existing controlnext weights")
        multimodalnet = MultiModalNet(token_size=config.model_token_size, **model_kwargs)
        state_dict = torch.load(config.multimodalnet_path, map_location=lambda storage, loc: storage)
        missing, unexpected = multimodalnet.load_state_dict(state_dict['state_dict'], strict=False)
    else:
        logger.info("Initializing controlnext weights from unet")
        multimodalnet = MultiModalNet(token_size=config.model_token_size, **model_kwargs)
    multimodalnet = multimodalnet.train()
    multimodalnet.requires_grad_(True)

    model = SewingLDM(dit, multimodalnet, control_scale=config.control_scale, idx=args.idx).train()

    logger.info(f"{dit.__class__.__name__} Model Parameters: {sum(p.numel() for p in dit.parameters() if p.requires_grad):,}")
    logger.info(f"{multimodalnet.__class__.__name__} Model Parameters: {sum(p.numel() for p in multimodalnet.parameters()):,}")
    logger.info(f"T5 max token length: {config.model_max_length}")

    if config.load_sewingldm_from is not None:
        missing, unexpected = load_checkpoint(
            config.load_sewingldm_from, model, load_ema=config.get('load_ema', False), max_length=max_length)
        logger.warning(f'Missing keys: {missing}')
        logger.warning(f'Unexpected keys: {unexpected}')
    ...
    # build dataloader
    config.data['load_probability'] = 0.75
    set_data_root(config.data_root)
    dataset = build_dataset(config.data, resolution=image_size, max_length=max_length, config=config)
