...
def save_checkpoint(work_dir,
    ...
    if save_last:
        last_file_path = os.path.join(work_dir, 'last.pth')
        torch.save(state_dict, last_file_path)
        logger.info(f'Saved last checkpoint to {last_file_path}.')
    else:
...
def load_checkpoint(checkpoint,
    ...
    if os.path.exists(f'output/pretrained_models/null_embed_diffusers_{max_length}token.pth'):
