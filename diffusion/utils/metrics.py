...
# My modules
from diffusion.data.datasets.dataset import GarmentDetrDataset as PatternDataset
from .eval_detr_metrics import * 
    ...
    def __call__(
        ...
        for pattern_idx in range(stitch_tags.shape[0]):
            ...
            for detected in stitch_list.transpose(0, 1):
                for actual in gt_stitches[pattern_idx][:, :int(gt_stitches_nums[pattern_idx])].transpose(0, 1):
...
class NumbersInPanelsAccuracies():
    ...
    def __call__(self, predicted_outlines, gt_num_edges, gt_panel_nums, pattern_names=None):
        ...
        return (
            ...
            num_edges_in_correct / (correct_pattern_mask.sum() + 1e-9)  # edges for correct patterns
        )
