...
class ComposedLoss():
    ...
    def __init__(self, data_config, in_config={}):
        ...
        self.with_quality_eval = False  # quality evaluation switch -- may allow to speed up the loss evaluation if False
...
class ComposedPatternLoss():
    ...
    def __init__(self, data_config, in_config={}):
        ...
        if 'shape' in self.l_components or 'rotation' in self.l_components or 'translation' in self.l_components:
            self.regression_loss = nn.MSELoss(reduce=None)  
        if 'loop' in self.l_components:
            self.loop_loss = PanelLoopLoss(self.max_panel_len, data_stats=self.gt_outline_stats) # sum vector and divide by scale
        ...
        if 'edge_type' in self.l_components:
            self.edge_type_loss = nn.BCEWithLogitsLoss()
        if 'edge_mask' in self.l_components:
            self.edge_mask_loss = nn.BCEWithLogitsLoss()
    ...
    def _main_losses(self, preds, ground_truth, gt_num_edges):
        ...
        full_loss = 0.
        loss_dict = {}
        edges_mask = ~ground_truth['empty_edges_mask']
        non_zero_edges = edges_mask.float().sum()
        panels_mask = ~ground_truth['empty_panels_mask']
        non_zero_panels = panels_mask.float().sum()

        if 'shape' in self.l_components:
            if "aug_outlines" in ground_truth:
                aug_pred_outlines = self.pred_to_aug_outlines(preds['outlines'])
                pattern_loss = self.regression_loss(aug_pred_outlines, ground_truth["aug_outlines"]) * edges_mask.float()
                pattern_loss = pattern_loss.sum() / non_zero_edges * self.config['loss_weight_dict']['shape_loss_weight']
            else:
                pattern_loss = self.regression_loss(preds['outlines'], ground_truth['outlines'])
                pattern_loss = pattern_loss.mean() * self.config['loss_weight_dict']['shape_loss_weight']
        ...
        if 'loop' in self.l_components:
            ...
            full_loss += self.config['loop_loss_weight'] * loop_loss * self.config['loss_weight_dict']['loop_loss_weight']
            loss_dict.update(loop_loss=loop_loss)
            
        if 'rotation' in self.l_components:
            # independent from panel loop origin by design
            rot_loss = self.regression_loss(preds['rotations'], ground_truth['rotations']) * panels_mask.float()
            rot_loss = rot_loss.sum() / non_zero_panels * self.config['loss_weight_dict']['rotation_loss_weight']
            full_loss += rot_loss
            loss_dict.update(rotation_loss=rot_loss)
        
        if 'translation' in self.l_components:
            # independent from panel loop origin by design
            translation_loss = self.regression_loss(preds['translations'], ground_truth['translations']) * panels_mask.float()
            translation_loss = translation_loss.sum() / non_zero_panels * self.config['loss_weight_dict']['translation_loss_weight']
        ...
        if 'edge_type' in self.l_components:
            edge_type_loss = self.edge_type_loss(10 * preds['edge_type'], ground_truth['edge_type'].type(torch.FloatTensor).to(self.device))
            full_loss += edge_type_loss
            loss_dict.update(edge_type_loss=edge_type_loss)
        
        if 'edge_mask' in self.l_components:
            edge_mask_loss = self.edge_mask_loss(10 * preds['edge_mask'], (~ground_truth['empty_edges_mask']).type(torch.FloatTensor).to(self.device))
            full_loss += edge_mask_loss
            loss_dict.update(edge_mask_loss=edge_mask_loss)
    ...
    def _stitch_losses(self, preds, ground_truth, gt_num_edges):
        ...
        if 'stitch' in self.l_components: 
            ...
            stitch_loss, stitch_loss_breakdown = self.stitch_loss(
                preds['stitch_tags'], ground_truth['stitches'], ground_truth['num_stitches'].to(torch.int))
            loss_dict.update(stitch_loss_breakdown)
            full_loss += stitch_loss
        # print("stitch loss time: ", time.time() - start_time)
        if 'stitch_supervised' in self.l_components:
        ...
        if 'free_class' in self.l_components:
            ...
            free_edges_loss = self.bce_logits_loss(
                10* preds['free_edges_mask'], ground_truth['free_edges_mask'].type(torch.FloatTensor).to(self.device))
            reverse_edges_loss = self.bce_logits_loss(
                10 * preds['reverse_stitch'], ground_truth['reverse_stitch'].type(torch.FloatTensor).to(self.device))
            loss_dict.update(free_edges_loss=free_edges_loss)
            loss_dict.update(reverse_edges_loss=reverse_edges_loss)
            full_loss += (free_edges_loss + reverse_edges_loss) / 2
