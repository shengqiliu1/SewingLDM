...
class NNSewingPattern(VisPattern):
    ...
    """
    def __init__(self, pattern_file=None, panel_classifier=None):
        """
    ...
    def pattern_as_tensors(
        ...
        if with_stitch_tags:
            ...
            stitch_per_edge = np.zeros((len(panel_seqs), len(panel_seqs[0]), 1))
            reverse_stitch = np.zeros((len(panel_seqs), len(panel_seqs[0]), 1))

        for idx, stitch in enumerate(self.pattern['stitches']):
            right_wrong = False
            if len(stitch) > 2:
                right_wrong = True
            for id_side, side in enumerate(stitch[:2]):
                ...
                if with_stitch_tags:
                    ...
                    stitch_per_edge[panel_id][edge_id] = 1
                if right_wrong:
                    reverse_stitch[panel_id][edge_id] = 1
        ...
        if with_stitch_tags:
            ...
            result.append(stitch_per_edge)
            result.append(reverse_stitch)

        return tuple(result) if len(result) > 1 else result[0]

    def pattern_from_tensors(
            self, pattern_representation, 
            edge_type=None, edge_mask=None, panel_rotations=None, panel_translations=None, stitches=None, reverse_stitch=None,
            padded=False):
        ...
        for idx in range(len(pattern_representation)):
            ...
            try:
                self.panel_from_numeric(
                    ...
                    edge_type=edge_type[idx] if edge_type is not None else None,
                    edge_mask=edge_mask[idx] if edge_mask is not None else None,
        ...
        reverse_stitch = torch.round(torch.sigmoid(reverse_stitch)).type(torch.int) if reverse_stitch is not None else None
        if stitches is not None and len(stitches) > 0:
            ...
            for stitch_id in range(stitches.shape[1]):
                ...
                if reverse_stitch is not None and reverse_stitch[in_panel_id][int(stitches[0][stitch_id] % edges_per_panel)] and reverse_stitch[in_panel_id][int(stitches[1][stitch_id] % edges_per_panel)]:
                    stitch_object.append('right_wrong')
    ...
    def _control_to_abs_coord_batch(start, end, control_scale):
        ...
        ! todo circle and cubic Bezier control point
        """
        edge = end - start
    ...
    def panel_from_numeric(self, panel_name, edge_sequence, edge_type=None, edge_mask=None, rotation=None, translation=None, padded=False):
        ...
        edge_type = torch.round(torch.sigmoid(edge_type)).type(torch.int16).cpu().numpy() if edge_type is not None else None
        edge_mask = torch.round(torch.sigmoid(edge_mask)).type(torch.BoolTensor).cpu().numpy() if edge_mask is not None else None

        if padded:
            if edge_mask is not None:
                edge_sequence = edge_sequence[edge_mask]
        ...
        for idx in range(len(edge_sequence) - 1):
            ...
            edges.append(self._edge_dict(idx, idx + 1, edge_info[2:], edge_type[idx]))
        ...
        if all(np.isclose(fin_vert, 0, atol=3)):  # 3 cm per coordinate is a tolerable error
            edges.append(self._edge_dict(idx, 0, edge_info[2:], edge_type[idx]))
        else:
    ...
    def _edge_dict(self, vstart, vend, curvature, edge_type):
        """Convert given info into the proper edge dictionary representation"""
        edge_dict = {'endpoints': [vstart, vend]}
        line_type = edge_type[2:4]
        if line_type[0] == 0 and line_type[1] == 0:
            pass
        elif line_type[0] == 0 and line_type[1] == 1:
            edge_dict['curvature'] = {'type': 'quadratic', 'params': [curvature[:2].tolist()]}
        elif line_type[0] == 1 and line_type[1] == 0:
            edge_dict['curvature'] = {'type': "circle", 'params':curvature[6:].tolist() + edge_type[:2].tolist()}
        elif line_type[0] == 1 and line_type[1] == 1:
            edge_dict['curvature'] = {'type': "cubic", 'params':curvature[2:6].reshape(2, 2).tolist()}
        
        label_type = edge_type[4:]
        if len(label_type) != 0:
            if label_type[0] == 0 and label_type[1] == 0 and label_type[2] == 1:
                edge_dict['label'] = 'lower_interface'
            elif label_type[0] == 0 and label_type[1] == 1 and label_type[2] == 0:
                edge_dict['label'] = 'right_collar'
            elif label_type[0] == 0 and label_type[1] == 1 and label_type[2] == 1:
                edge_dict['label'] = 'left_collar'
            elif label_type[0] == 1 and label_type[1] == 0 and label_type[2] == 0:
                edge_dict['label'] = 'strapless_top'
            elif label_type[0] == 1 and label_type[1] == 0 and label_type[2] == 1:
                edge_dict['label'] = 'right_armhole'
            elif label_type[0] == 1 and label_type[1] == 1 and label_type[2] == 0:
                edge_dict['label'] = 'left_armhole'
    ...
    def _empty_panel(self, max_edge_num):
        ...
        # edge is 9-elem vector+2-circle_type+3-attach type, 4 rotation element for quaternion, 3 element for world translation
        return np.zeros((max_edge_num, 11+2+3)), np.zeros(4), np.zeros(3)
