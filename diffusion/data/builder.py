...
DATAWRAPPERS = Registry('datawrappers')
...
def build_dataset(cfg, resolution=224, **kwargs):
    ...
    logger.info(f"Dataset {dataset_type} constructed. time: {(time.time() - t):.2f} s, length (use/ori): {len(dataset)}")
    return dataset

def build_datawrapper(type, dataset, batch_size=256, num_workers=4, shuffle=True, **kwargs):
    logger = get_root_logger()
    logger.info(f"Constructing Datawrapper {type}...")
    t = time.time()
    build_cfg = dict(type=type, in_dataset=dataset)
    datawrapper = build_from_cfg(build_cfg, DATAWRAPPERS, 
                                 default_args=dict(batch_size=batch_size, num_workers=num_workers, shuffle_train=shuffle, **kwargs))
    logger.info(f"Split and standardize_data constructed. time: {(time.time() - t):.2f}")
