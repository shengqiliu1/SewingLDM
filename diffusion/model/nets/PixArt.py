...
class PixArt(nn.Module):
    ...
    def __init__(
        ...
        if config:
            ...
            logger.warning(f"position embed interpolation: {self.pe_interpolation}")
            logger.warning(f"kv compress config: {self.kv_compress_config}")
        else:
            print(f'Warning: position embed interpolation: {self.pe_interpolation}')
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
        ...
        y = self.y_embedder(y, self.training)  # (N, 1, 120, D)
        if mask is not None:
    ...
    def forward_with_dpmsolver(self, x, timestep, y, mask=None, **kwargs):
        ...
        if self.pred_sigma:
            return model_out.chunk(2, dim=2)[0]
        else:
            return model_out
...
def PixArt_XL_2(**kwargs):
    return PixArt(depth=24, num_heads=12, **kwargs)
