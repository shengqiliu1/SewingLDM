...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        ...
        if self.model_var_type in [
            ...
            assert output.shape == (B, x_t.shape[1], C * 2)
            output, model_var_values = th.split(output, C, dim=2)
            # Learn the variance using the variational bound, but don't let it affect our mean prediction.
            frozen_out = th.cat([output.detach(), model_var_values], dim=2)
            ...
            pred_xstart = vb_terms["pred_xstart"]
        ...
        if isinstance(model_output, dict) and model_output.get('gamma', None) is not None:
            coff = model_output['gamma'].numel()
            terms['l1'] = th.norm(model_output['gamma'], 1) / coff + th.norm(model_output['var'], 1) / coff
        return terms, pred_xstart
